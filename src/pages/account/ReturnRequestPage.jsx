...
import { createCustomerReturnRequest, getCustomerReturnRequest, RETURN_REQUEST_TYPES } from "../../services/returnRequestServices";

const REASONS = ["Wrong size", "Wrong product or color", "Damaged on delivery", "Defective product", "Missing item", "Other"];
...
function ReturnRequestPage() {
  ...
  useEffect(() => {
    if (!loading && !isAuthenticated) navigate("/account/login", { replace: true, state: { from: `/account/orders/${orderId}/return` } });
  ...
  async function handleSubmit(event) {
    ...
    setError("");
    try {
      const result = await createCustomerReturnRequest(user.$id, orderId, { requestType, reason, details, itemIds: selectedItems, exchangeNote });
  ...
  if (!orderData) return <main className="account-page"><div className="account-page__container"><div className="return-request__empty"><Package size={32} /><h1>Order not found.</h1><Link to="/account/orders">Back to orders</Link></div></div></main>;
  ...
  if (existingRequest) {
    return (
      <main className="account-page"><div className="account-page__container"><section className="return-request__card">
        <Link to={`/account/orders/${orderId}`} className="return-request__back"><ArrowLeft size={16} />Back to order</Link>
        <div className="return-request__success"><CheckCircle2 size={34} /><span>Return request</span><h1>{existingRequest.return_Number}</h1><p>Your {existingRequest.request_Type} request is being handled by the store team.</p><strong>Status: {existingRequest.status}</strong></div>
      </section></div></main>
    );
  }
  ...
  return (
    <main className="account-page"><div className="account-page__container"><section className="return-request__card">
      <Link to={`/account/orders/${orderId}`} className="return-request__back"><ArrowLeft size={16} />Back to order</Link>
      <header className="return-request__header"><span>Order {orderData.order.order_Number || orderId}</span><h1>Return or exchange.</h1><p>Select the affected item(s) and tell us what happened. Final eligibility and resolution are handled by the store team.</p></header>
      {error && <div className="return-request__error">{error}</div>}
      <form onSubmit={handleSubmit} className="return-request__form">
        <fieldset><legend>Request type</legend><div className="return-request__choices">
        ...
        <fieldset><legend>Items</legend><div className="return-request__items">
          {orderData.items.map((item) => <label key={item.$id}><input type="checkbox" checked={selectedItems.includes(item.$id)} onChange={() => toggleItem(item.$id)} /><span><strong>{item.product_Name}</strong><small>{item.product_Color ? `Color: ${item.product_Color} · ` : ""}Qty: {item.quantity}</small></span></label>)}
        </div></fieldset>
