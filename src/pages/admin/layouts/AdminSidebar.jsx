    ...
    <aside className="hidden w-64 shrink-0 border-r border-black/8 bg-white lg:block">
      <div className="sticky top-0 flex h-screen flex-col">
        ...
        <nav className="flex-1 px-3 py-6">
          <p className="mb-3 px-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-black/35">Management</p>
          <div className="space-y-1">
            {navigation.filter((item) => canAccess(item.area)).map((item) => {
        ...
        <div className="border-t border-black/8 p-4">
          <button type="button" onClick={handleSignOut} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-black/55 transition hover:bg-black/5 hover:text-black">
