...
function ReturnRequestsPage() {
  ...
  async function handleResolutionChange(request, nextResolution) {
    setSavingId(request.$id);
    setError("");
    try {
      const updated = await updateAdminReturnRequest(request.$id, request.status, request.status, { resolution: nextResolution, refundAmount: request.refund_Amount, managementNote: request.management_Note });
      setRequests((current) => current.map((item) => item.$id === updated.$id ? updated : item));
    } catch (saveError) {
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <CustomDropdown
          ...
          options={[
            ...
            ...Object.keys(ADMIN_RETURN_STATUS_TRANSITIONS).map((status) => ({
              value: status,
              label: status,
            })),
          ]}
              ...
              <article key={request.$id} className="rounded-lg border border-black/8 bg-white p-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <CustomDropdown
                    ...
                    onChange={(value) => handleStatusChange(request, value)}
                    disabled={disabled}
                ...
                <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <label className="block"><span className="text-xs text-black/40">Resolution</span><CustomDropdown
                      value={request.resolution || "pending"}
                      disabled={disabled}
                      onChange={(value) => handleResolutionChange(request, value)}
