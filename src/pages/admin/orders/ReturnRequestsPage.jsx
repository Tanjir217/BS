...
import { ADMIN_RETURN_STATUS_TRANSITIONS, getAdminReturnRequests, updateAdminReturnRequest } from "../../../services/adminReturnRequestServices";
...
function ReturnRequestsPage() {
  ...
  async function handleStatusChange(request, event) {
    ...
    try {
      const updated = await updateAdminReturnRequest(request.$id, request.status, nextStatus, { resolution: request.resolution, refundAmount: request.refund_Amount, managementNote: request.management_Note });
  ...
  async function handleResolutionChange(request, event) {
    ...
    setError("");
    try {
      const updated = await updateAdminReturnRequest(request.$id, request.status, request.status, { resolution: event.target.value, refundAmount: request.refund_Amount, managementNote: request.management_Note });
      setRequests((current) => current.map((item) => item.$id === updated.$id ? updated : item));
    } catch (saveError) {
  ...
  async function handleRefundAmountChange(request, event) {
    ...
    setError("");
    try {
      const updated = await updateAdminReturnRequest(request.$id, request.status, request.status, { resolution: request.resolution, refundAmount: value, managementNote: request.management_Note });
      setRequests((current) => current.map((item) => item.$id === updated.$id ? updated : item));
    } catch (saveError) {
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="rounded-md border border-black/10 bg-white px-3 py-2 text-sm"><option value="all">All requests</option>{Object.keys(ADMIN_RETURN_STATUS_TRANSITIONS).map((status) => <option key={status} value={status}>{status}</option>)}</select>
      </div>

      {error && <div className="mb-5 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error}</div>}
      {loading ? <div className="rounded-lg border border-black/8 bg-white p-8 text-sm text-black/50">Loading return requests...</div> : requests.length === 0 ? <div className="rounded-lg border border-black/8 bg-white p-8 text-sm text-black/50">No return or exchange requests found.</div> : (
            ...
            return (
              <article key={request.$id} className="rounded-lg border border-black/8 bg-white p-5">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  <div><p className="text-xs uppercase tracking-wide text-black/40">{request.request_Type}</p><h2 className="mt-1 text-lg font-semibold">{request.return_Number}</h2><p className="mt-1 text-sm text-black/50">Order: {request.order_ID}</p></div>
                  <select value={request.status} disabled={disabled} onChange={(event) => handleStatusChange(request, event)} className="rounded-md border border-black/10 bg-white px-3 py-2 text-sm">{allowedStatuses.map((status) => <option key={status} value={status}>{status}</option>)}</select>
                </div>
                <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  <div><p className="text-xs text-black/40">Reason</p><p className="mt-1 text-sm">{request.reason || "—"}</p></div>
                  <div><p className="text-xs text-black/40">Selected items</p><p className="mt-1 break-words text-sm">{request.requested_Item_IDs || "—"}</p></div>
