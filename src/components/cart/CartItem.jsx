...
function CartItem({ item }) {
  ...
  const hasPriceChanged = item.validationStatus === "price-changed";

  const hasStockReduced = item.validationStatus === "stock-reduced";

  const maxQuantity =
    Number(item.stockQuantity) > 0 ? Number(item.stockQuantity) : null;

  return (
    <article className={`cart-item ${isUnavailable ? "is-unavailable" : ""}`}>
      ...
      <div className="cart-item__content">
        <div className="cart-item__header">
          <div>
            ...
            {item.color && (
              <p className="cart-item__meta">Colour: {item.color}</p>
            )}
          </div>
        ...
        {hasPriceChanged && (
          <div className="cart-item__notice">
            ...
            <span>Price updated to ৳{formatPrice(item.price)}</span>
          </div>
        )}
        ...
        {hasStockReduced && (
          <div className="cart-item__notice">
            ...
            <span>
              Quantity adjusted to {item.quantity}
        ...
        <div className="cart-item__bottom">
          ...
          <p className="cart-item__price">
            {isUnavailable ? (
              <span className="cart-item__unavailable-price">Unavailable</span>
            ) : (
