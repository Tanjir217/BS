...
function InspiredProductSlider({ products = [] }) {
  ...
  const visibleProducts = useMemo(() => {
    ...
    return Array.from({ length: visibleCount }, (_, slot) => {
      const index =
        (activeIndex + slot - centerSlot + products.length) % products.length;
  ...
  return (
    ...
    >
      <h2 id="inspired-products-title" className="inspired-slider__title">
        Get inspired
      </h2>
      <div className="inspired-slider__viewport" {...swipeHandlers}>
        <button
          aria-label="Previous products"
          className="inspired-slider__arrow inspired-slider__arrow--previous"
          onClick={() => move(-1)}
          type="button"
        >
        ...
        >
          {visibleProducts.map((product, index) => {
            ...
            return (
              <article
                className={`inspired-slider__item ${isFeatured ? "is-featured" : ""}`}
                key={`${product.id}-${index}`}
              >
                <Link
                  aria-label={`View ${product.name}`}
                  className="inspired-slider__image"
                  to={product.href}
                >
                  <img
                    alt=""
        ...
        <button
          ...
          className="inspired-slider__arrow inspired-slider__arrow--next"
          onClick={() => move(1)}
          type="button"
        >
      ...
      <div className="inspired-slider__details">
        ...
        <Link className="inspired-slider__cta" to={featuredProduct.href}>
          Shop now
        </Link>
      </div>
    </section>
